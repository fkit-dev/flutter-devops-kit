...
class ModuleService {
  ...
  Future<ModuleDefinition> load({required String template, required String module}) async {
    final root = const PackageLocator().packageRoot();

    final file = File('${root.path}/templates/$template/modules/$module/module.yaml');

    if (!file.existsSync()) throw Exception('Module "$module" not found in template "$template".');
