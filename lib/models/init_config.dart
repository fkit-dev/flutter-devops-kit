...
class InitConfig {
  ...
  factory InitConfig.fromMap(
    ...
    final tooling = Map<String, dynamic>.from(
      map['tooling'] ?? {},
    );
    ...
    final build = Map<String, dynamic>.from(
      map['build'] ?? {},
    );
    ...
    final flavoring = Map<String, dynamic>.from(
      map['flavoring'] ?? {},
    );

    final localization = Map<String, dynamic>.from(
      map['localization'] ?? {},
    );

    final firebase = Map<String, dynamic>.from(
      map['firebase'] ?? {},
    );
    ...
    final flavorsMap = Map<String, dynamic>.from(
      map['flavors'] ?? {},
    );
    ...
    for (final entry in flavorsMap.entries) {
      ...
      flavors[entry.key] = FlavorDetails.fromMap(
